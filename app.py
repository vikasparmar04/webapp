...
connection = pymongo.MongoClient('''mongodb+srv://vikasparmar04:vikas0409@cluster0.48pf6.mongodb.net/test?retryWrites=true&w=majority''')
db = connection.test # store a reference to the database
...
def read():
    ...
    """
    docs = post_collection.find({}).sort("created_at", -1) # sort in descending order of created_at timestamp
...
def create_post():
    ...
    post_collection.insert_one(doc) # insert a new document
...
def edit(mongoid):
    ...
    """
    doc = post_collection.find_one({"_id": ObjectId(mongoid)})
...
def delete(mongoid):
    ...
    """
    post_collection.delete_one({"_id": ObjectId(mongoid)})
